...
async def predict_multimodal(inp: MultimodalInput):
    ...
    try:
        numeric_inp = NumericInput(
            Poids=inp.Poids, Volume=inp.Volume,
            Conductivite=inp.Conductivite, Opacite=inp.Opacite,
            Rigidite=inp.Rigidite, Source=inp.Source,
        )
        ...
        return MultimodalOutput(
            categorie=categorie, prix_revente=price,
            confidence=conf, cluster_id=cluster_id,
            eco_score=compute_eco_score(categorie, conf, price),
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        ...
        if hasattr(clf, "feature_importances_") and clf_feats:
            ...
            for idx in indices[:8]:
                name = clf_feats[idx]
                display = name.replace("Categorie_", "Cat: ").replace("Source_", "Src: ")
            ...
            raw_vals = [inp.Poids, inp.Volume, inp.Conductivite, inp.Opacite, inp.Rigidite]
